...
STACKINFO_BEGIN
  ...
  if (mpIdHTTP)
  {
    ...
    try
    {
      lpStream = new TFileStream(aDest,Classes::fmCreate	);
...
STACKINFO_BEGIN
  ...
  if (mpIdHTTP)
  {
    ...
    else
    {
     ...
     try
     {
       lpStream = new TFileStream(aFrom,Sysutils::fmOpenRead		);
...
TPOP3Connect::TPOP3Connect(AnsiString aHost,int aPort,AnsiString aUserId,AnsiString aPassword,int aNbRetry,AnsiString aDir)
: TXXXConnect(aNbRetry)
{
TOTDebugInitFunc();
STACKINFO_BEGIN
...
void TSNMTConnect::FillAddr(TIdEMailAddressList  *apAddrList,TStringList *apStrList)
{
TOTDebugInitFunc();
    apAddrList->Clear();
    for(int li=0;li<apStrList->Count;li++)
    {
      ...
      lpItem->Address=apStrList->Strings[li];
      //lpIdMessage->Recipients->Add(lpList->Strings[li]);
    }

}

int TSNMTConnect::SendMail(AnsiString aSubject,TStringList * aMsg,
...
*/
  ...
  if (aAttachments!=AnsiString(""))
  {
    ...
    if (mUseIndy)
    {
	  ...
	  for(int li=0;li<lpList->Count;li++)
	  {
		TIdAttachment  *lpIdAttachment= new TIdAttachment(lpIdMessage->MessageParts,lpList->Strings[li]);
	  }
	  */
	}
...
//int ProcessMessage(TIdMessage *apMessage,,const AnsiString &aPath,AnsiString &aBody,AnsiString aAttach)
int TPOP3Connect::ProcessMessage(int aMsgNo,const AnsiString &aPath,AnsiString &aBody,AnsiString &aAttach)
{
TOTDebugInitFunc();
  int lj;
  aAttach="";
  ...
  for (lj=0;lj<lNbPart;lj++)
  {
     ...
     if(lpPart->InheritsFrom(__classid(  TIdAttachment)))
     {
        ...
        aAttach+=";";
        aAttach+=lpAttach->FileName;
        AnsiString lFileName=aPath;
        if (lFileName.SubString(lFileName.Length(),1)!=gDirectorySeparator)
        {
          lFileName+=gDirectorySeparator;
        }
