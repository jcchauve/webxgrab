...
var gHTTPGet_Pause : bool;
var gHTTPGet_Sleep : integer;
var gHTTPDownloaded_Bytes : integer;
...
type
  ...
  public
    ...
    constructor Create(aAcceptTypes, aAgent, aURL, aFileName, aUserName, aPassword, aPostQuery, aReferer: String;
                       aBinaryData, aUseCache: Boolean; aProgress: TOnProgressEvent; aToFile: Boolean; aTimeout : Integer;
                       aMinSize,aMaxSize : Integer
                       );
    property URL: String read FTURL;
    property FileName: String read FTFileName;
  end;
  ...
  published
    ...
    property  mThread : THTTPGetThread read FThread;
...
begin
  try
    ...
    InternetSetOption(hSession,
      INTERNET_OPTION_SEND_TIMEOUT, @FTimeout,4 );
    ...
    if FTResult or not FTBinaryData then
     begin
      ...
      if (FTFileSize<FTMinSize*1024)
      or ((FTFileSize>(FTMaxSize*1024)) and (FTMaxSize>=FTMinSize))
      Then
      Begin
          FreeMem(Buf);
          CloseHandles;
      ...
      while True do
       begin
        ...
        while (gHTTPGet_Pause) do 
        Begin
          Sleep(1000);
        End;
        if(gHTTPGet_Sleep>0) Then
        Begin
          Sleep(gHTTPGet_Sleep);
        End;
        if not InternetReadFile(hRequest, @Data, SizeOf(Data), BytesToRead) then Break
         ...
         if BytesToRead = 0 then Break
          ...
          begin
           ...
           inc(gHTTPDownloaded_Bytes,BytesToRead);
  ...
  if not Assigned(FThread) then
   begin
    FThread := THTTPGetThread.Create(FAcceptTypes, FAgent, FURL, FFileName, FUserName, FPassword, FPostQuery, FReferer,
                                     FBinaryData, FUseCache, FProgress, True,FTimeout,FMinSIze,FMaxSize);
...
begin
  if not Assigned(FThread) then
   begin
    FThread := THTTPGetThread.Create(FAcceptTypes, FAgent, FURL, FFileName, FUserName, FPassword, FPostQuery, FReferer,
                                     FBinaryData, FUseCache, FProgress, False,FTimeout,FMinSIze,FMaxSize);
    FThread.OnTerminate := ThreadDone;
    if FWaitThread then
     while PeekMessage(Msg, 0, 0, 0, PM_REMOVE) do
      begin
...
Begin
  gHTTPGet_Sleep:=0;
  gHTTPGet_Pause:=false;
  gHTTPDownloaded_Bytes:=0;
end.
